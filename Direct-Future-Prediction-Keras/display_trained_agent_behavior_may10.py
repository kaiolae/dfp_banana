...
if __name__ == "__main__":
    ...
    env = UnityEnv("../unity_envs/kais_banana2", worker_id=9, use_visual=True, uint8_visual=True, flatten_branched=True)
    ...
    measurement_size = 2 # [posion, food]
    ...
    m_t = np.array([food, poison])

    # Goal
    #TODO: How to sync loaded weights with agent goal?
    goal = np.array([-1.0, 1.0] * len(timesteps))
    #for both: goal = np.array([0.0, 1.0, -1.0] * len(timesteps))
    #TODO Make input argument
    loaded_model = "may9_after_fixing_goal_input_during_training_goal_agnostic//model/dfp.h5"
    ...
    for t in range(num_timesteps):
        ...
        m_t = np.array([food, poison]) # Measurement after transition
        s_t = s_t1
