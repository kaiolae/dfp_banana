...
class DFPAgent:
    ...
    def __init__(self, state_size, measurement_size, action_size, timesteps, goal_size):
    ...
    def get_action(self, state, measurement, goal, inference_goal):
        ...
        if np.random.rand() <= self.epsilon:
            ...
            #print("Prediction before stack: ", f)
            #print("In get action, prediction is ", f_pred)
            #print("And inference goal is ", inference_goal)
            #print("obj after multiply: ", obj)
            #KOE: Double-check the NN output a bit.
            action_idx = np.argmax(obj)
        return action_idx

    # Save trajectory sample <s,a,r,s'> to the replay memory
    #KOEChange: Originally, this didn't add the goal to memory. That doesn't work for goal-agnostic training. Added that here.
    #I confirmed that in the original DFP code, goals are indeed both saved and used during training.
    #I follow the original DFP paper in storing the entire (redundant) goal vector, with the goal repeated for every time step.
    def replay_memory(self, s_t, action_idx, r_t, s_t1, m_t, is_terminated, goal_t):
        self.memory.append((s_t, action_idx, r_t, s_t1, m_t, is_terminated, goal_t))
        if self.epsilon > self.final_epsilon and t > self.observe:
            self.epsilon -= (self.initial_epsilon - self.final_epsilon) / self.explore

        if len(self.memory) > self.max_memory: #KOETODO: We lose old experiences here. Could that limit training? Check how original DFP handled that.
            self.memory.popleft()
    ...
    def train_minibatch_replay(self):
        ...
        """
        #KOETODO: Training takes the goal we input here and learns to predict future. But that is wrong: We want to predict the effect
        #of the goal the agent was ACTUALLY OPERATING BY when the memory here was stored, don't we?
        ...
        # KOE: Here is the problem. We're using the input goal rather than the goal from memory. That does not allow goals to switch between episodes as in the original paper on agnostic mode.
        #goal_input = np.tile(goal, (batch_size, 1))
        #KOE: Changing so we load goal from memory instead.
        goal_input = np.zeros((batch_size, self.goal_size))
        ...
        for i, idx in enumerate(rand_indices):
            ...
            goal_input[i,:] = self.memory[idx][6]
            action.append(self.memory[idx][1])

        #print("Memory replay. meas is : ", measurement_input, ". goal is: ", goal_input)
...
if __name__ == "__main__":
    ...
    SAVE_TO_FOLDER = "may10_debugging_goal_agnostic"
    if goal_agnostic:
        SAVE_TO_FOLDER+="_goal_agnostic"
    else:
        SAVE_TO_FOLDER+="_not_goal_agnostic"
    ...
    env = UnityEnv("../unity_envs/kais_banana2", worker_id=29, use_visual=True, uint8_visual=True, flatten_branched=True)
    ...
    measurement_size = 2 # TODO: Increase after verifying that it works with just 2. [Battery, posion, food]
    number_of_goals = 2
    timesteps = [1,2,4,8,16,32]
    goal_size = number_of_goals * len(timesteps)
    ...
    agent = DFPAgent(state_size, measurement_size, action_size, timesteps, goal_size)
    ...
    m_t = np.array([food, poison]) #TODO: np.array([misc/100.0, food, poison])
    ...
    if goal_agnostic:
        goal_vector= [random.uniform(-1,1) for i in range(number_of_goals)]
    else:
        goal_vector = [1.0,-1.0] #TODO: [0.0, 1.0, -1.0]
    ...
    total_training_timesteps = timesteps_per_game*8000
    ...
    with open(SAVE_TO_FOLDER + "/goals_and_scores.txt", "a+") as stats_file:
        stats_file.write('GAME_NUMBER ')
        stats_file.write('food_goal ')
        stats_file.write('poison_goal ')
        stats_file.write('foods_gathered ')
        stats_file.write('poisons_gathered \n ')
    ...
    for t in range(total_training_timesteps):
        ...
        if (done):
            ...
            GAME += 1
            if goal_agnostic:
                #KOE: Setting up a simple measurement of goal-agnostic reward.
                food_reward = food
                if goal_vector[0] < 0: #The food-goal.
                    food_reward=-food
                poison_reward = poison
                if goal_vector[1] < 0: #The poison-goal
                    poison_reward = -poison
                reward_buffer.append(food_reward+poison_reward) #The reward we got scaled by goals, meaning high numbers indicate we followed the given goal well, low numbers mean we did the opposite.
            else:
            ...
            previous_goal_vector = copy.deepcopy(goal_vector)
            if goal_agnostic:
        ...
        agent.replay_memory(s_t, action_idx, r_t, s_t1, m_t, done, goal) #KOE: I added storing and using goals in the replay memory.

        #KOETODO: Think about normalization.
        m_t = np.array([food, poison]) #TODO: np.array([meas[0][0]/100.0, food, poison]) # Measurement after transition

        # Do the training
        if t > agent.observe and t % agent.timestep_per_train == 0:
            loss = agent.train_minibatch_replay()
        ...
        if (done):
            ...
            #KOETODO: Remove later. Using to debug why goal agnostic did not work.
            with open(SAVE_TO_FOLDER + "/goals_and_scores.txt", "a+") as stats_file:
                stats_file.write(str(GAME) + ' ')
                stats_file.write(str(previous_goal_vector[0]) + ' ')
                stats_file.write(str(previous_goal_vector[1]) + ' ')
                stats_file.write(str(food) + ' ')
                stats_file.write(str(poison) + ' \n')
